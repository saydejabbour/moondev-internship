...
export default function LoginClient({ nextParam }: { nextParam: string }) {
  ...
  // memoize in case parent re-renders
  const nextSafeMemo = useMemo(() => nextParam || '', [nextParam]);
  ...
  // NOTE: this assumes your profiles table uses column "user_id" (common pattern).
  // If your column is "id", switch user_id -> id in the queries below.
  ...
  const createProfileIfMissing = async (
    ...
    const { error } = await supabase
      .from('profiles')
      .insert({ user_id: userId, role: roleFromMeta, full_name: '' });
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const metaRole =
        (user.user_metadata?.role as Role | undefined) ?? 'developer';
      const role = await createProfileIfMissing(user.id, metaRole);
      router.replace(destinationFor(role, nextSafeMemo));
    })().catch(() => {
      /* ignore on load */
    });
    return () => {
    ...
    const redirectTo =
      typeof window !== 'undefined'
        ? `${window.location.origin}/auth/callback${
            isSafeNext(nextSafeMemo)
              ? `?next=${encodeURIComponent(nextSafeMemo)}`
              : ''
          }`
        : undefined;
