...
export default function LoginClient({ nextParam }: { nextParam: string }) {
  ...
  // Keep a stable, sanitized next value
  const nextSafe = useMemo(() => nextParam || '', [nextParam]);
  ...
  // NOTE: your "profiles" table uses "id" (PK) that equals auth.uid()
  ...
  const createProfileIfMissing = async (userId: string, roleFromMeta: Role) => {
    ...
    const { error } = await supabase
      .from('profiles')
      .insert({ id: userId, role: roleFromMeta, full_name: '' }); // <-- id

    if (error) throw error;
    return roleFromMeta;
  };

  // If already signed in, ensure profile row exists and redirect
  useEffect(() => {
    let mounted = true;
    (async () => {
      ...
      router.replace(destinationFor(role, nextSafe));
    })().catch(() => { /* ignore */ });
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!role) role = await createProfileIfMissing(user.id, metaRole);
    ...
    const redirectTo =
      typeof window !== 'undefined'
        ? `${window.location.origin}/auth/callback${
            isSafeNext(nextSafe) ? `?next=${encodeURIComponent(nextSafe)}` : ''
          }`
        : undefined;
